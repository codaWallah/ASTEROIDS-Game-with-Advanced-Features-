...
const NUM_STARS = 170; // Number of stars for the background
...
class Ship {
  ...
  draw() {
    ...
    // --- Booster / engine glow (rear, single nozzle for Tejas) ---
    if (this.boosterFlameLength > 0) {
      const flicker = Math.random() * 0.22 + 0.9;
      const flameLen = this.boosterFlameLength * flicker;
      const outer = ctx.createLinearGradient(
        -SHIP_SIZE / 2 - flameLen,
        0,
        -SHIP_SIZE / 4,
        0
      );
      outer.addColorStop(0, 'rgba(255,160,40,0)');
      outer.addColorStop(0.5, 'rgba(255,120,30,0.6)');
      outer.addColorStop(1, 'rgba(255,210,100,0.95)');
      ctx.beginPath();
      ctx.moveTo(-SHIP_SIZE / 2.4, SHIP_SIZE / 5);
      ctx.lineTo(-SHIP_SIZE / 2 - flameLen, 0);
      ctx.lineTo(-SHIP_SIZE / 2.4, -SHIP_SIZE / 5);
      ctx.closePath();
      ctx.fillStyle = outer;
      ctx.fill();

      const core = ctx.createRadialGradient(
        -SHIP_SIZE / 2 - flameLen * 0.36,
        0,
        0,
        -SHIP_SIZE / 2 - flameLen * 0.36,
        0,
      ...
      core.addColorStop(1, 'rgba(255,160,30,0.8)');
      ctx.beginPath();
      ctx.ellipse(
        -SHIP_SIZE / 2 - flameLen * 0.34,
        0,
        flameLen * 0.28,
        flameLen * 0.14,
        0,
    ...
    // --- Tejas-inspired main body (blended delta with LERX, single fin, side intakes) ---
    const noseX = SHIP_SIZE * 0.78;
    const rearX = -SHIP_SIZE * 0.9;
    const fusHalf = SHIP_SIZE * 0.17;
    const wingRootX = SHIP_SIZE * 0.22;   // where wing meets fuselage
    const wingTipY = SHIP_SIZE * 0.68;    // half-span
    const lerxX = SHIP_SIZE * 0.34;       // LERX outer point
    const intakeX = SHIP_SIZE * 0.06;     // intake forward offset

    // Fuselage + blended delta wing outline
    ctx.beginPath();
    ctx.moveTo(noseX, 0); // sharp nose
    // upper leading edge out to wingtip
    ...
    // rear upper to center
    ctx.quadraticCurveTo(
      rearX + SHIP_SIZE * 0.04,
      -fusHalf * 1.05,
      rearX,
      0
    );
    // mirror lower surface
    ctx.quadraticCurveTo(
      rearX + SHIP_SIZE * 0.04,
      fusHalf * 1.05,
      wingRootX,
    ...
    // Body gradient (top darker, belly lighter)
    const bodyGrad = ctx.createLinearGradient(0, -SHIP_SIZE * 0.6, 0, SHIP_SIZE * 0.6);
    bodyGrad.addColorStop(0, '#3d505f'); // top
    bodyGrad.addColorStop(0.45, '#7fb0d6');
    bodyGrad.addColorStop(1, '#e3f6ff'); // belly

    ctx.fillStyle = bodyGrad;
    ctx.lineWidth = 1.4;
    ctx.strokeStyle = 'rgba(14,64,94,0.95)';
    ctx.fill();
    ctx.stroke();

    // LERX (leading-edge root extensions) - characteristic blended shape
    ctx.fillStyle = 'rgba(18,34,48,0.95)';
    ctx.beginPath();
    ctx.moveTo(SHIP_SIZE * 0.14, -SHIP_SIZE * 0.05);
    ctx.lineTo(lerxX, -SHIP_SIZE * 0.46);
    ctx.lineTo(lerxX - SHIP_SIZE * 0.06, -SHIP_SIZE * 0.12);
    ctx.closePath();
    ctx.fill();
    ...
    // Side air intakes (subtle, on fuselage sides)
    ctx.fillStyle = 'rgba(0,0,0,0.22)';
    ctx.beginPath();
    ctx.ellipse(intakeX, -SHIP_SIZE * 0.16, SHIP_SIZE * 0.14, SHIP_SIZE * 0.07, -0.15, Math.PI * 0.2, Math.PI * 0.85);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(intakeX, SHIP_SIZE * 0.16, SHIP_SIZE * 0.14, SHIP_SIZE * 0.07, 0.15, Math.PI * 0.2, Math.PI * 0.85);
    ctx.fill();

    // Small close-coupled canards (very shallow on Tejas)
    const canardOffset = SHIP_SIZE * 0.28;
    ctx.fillStyle = 'rgba(28,62,98,0.96)';
    ctx.beginPath();
    ctx.moveTo(noseX - canardOffset, -fusHalf * 0.5);
    ctx.lineTo(noseX - canardOffset - SHIP_SIZE * 0.11, -fusHalf * 0.9);
    ctx.lineTo(noseX - canardOffset + SHIP_SIZE * 0.02, -fusHalf * 0.08);
    ctx.closePath();
    ctx.fill();
    ...
    // Single vertical fin (swept, centered)
    const finBaseX = rearX + SHIP_SIZE * 0.06;
    const finHeight = SHIP_SIZE * 0.36;
    ctx.fillStyle = 'rgba(30,68,108,0.98)';
    ctx.beginPath();
    ctx.moveTo(finBaseX, -fusHalf * 0.18);
    ctx.lineTo(finBaseX + SHIP_SIZE * 0.42, -finHeight);
    ctx.lineTo(finBaseX + SHIP_SIZE * 0.10, -fusHalf * 0.03);
    ctx.closePath();
    ctx.fill();

    // Engine nozzle (single) and subtle shroud
    const engineX = rearX + SHIP_SIZE * 0.06;
    const engineR = SHIP_SIZE * 0.14;
    const engGrad = ctx.createRadialGradient(engineX, 0, 1, engineX - 6, 0, engineR * 1.6);
    engGrad.addColorStop(0, '#ffffff');
    engGrad.addColorStop(0.3, '#ffdca0');
    engGrad.addColorStop(1, '#333333');
    ctx.beginPath();
    ctx.ellipse(engineX - 2, 0, engineR, engineR * 0.6, 0, 0, Math.PI * 2);
    ...
    // Thrust glow under nozzle when thrusting
    if (this.boosterFlameLength > 0) {
      const glow = ctx.createRadialGradient(engineX - engineR * 0.6, 0, 0, engineX - engineR * 1.6, 0, engineR * 3);
      glow.addColorStop(0, 'rgba(255,180,60,0.9)');
      glow.addColorStop(1, 'rgba(255,120,20,0)');
      ctx.beginPath();
      ctx.ellipse(engineX - engineR * 0.9, 0, engineR * 1.9, engineR * 0.95, 0, 0, Math.PI * 2);
      ctx.fillStyle = glow;
      ctx.fill();
    }

    // Cockpit / teardrop canopy (forward, typical fighter style)
    ctx.save();
    ctx.translate(noseX - SHIP_SIZE * 0.22, -SHIP_SIZE * 0.02);
    ctx.scale(1.02, 0.92);
    const glassGrad = ctx.createLinearGradient(-SHIP_SIZE * 0.12, 0, SHIP_SIZE * 0.12, 0);
    glassGrad.addColorStop(0, 'rgba(200,240,255,0.98)');
    glassGrad.addColorStop(0.6, 'rgba(110,180,220,0.9)');
    glassGrad.addColorStop(1, 'rgba(45,100,140,0.95)');
    ctx.beginPath();
    ctx.ellipse(0, 0, SHIP_SIZE * 0.15, SHIP_SIZE * 0.085, -0.08, 0, Math.PI * 2);
    ctx.fillStyle = glassGrad;
    ctx.fill();
    ctx.lineWidth = 0.9;
    ctx.strokeStyle = 'rgba(18,56,92,0.9)';
    ctx.stroke();
    ctx.restore();

    // Panel lines and center highlights
    ctx.strokeStyle = 'rgba(255,255,255,0.06)';
    ctx.lineWidth = 0.55;
    ctx.beginPath();
    ctx.moveTo(noseX - SHIP_SIZE * 0.08, -SHIP_SIZE * 0.02);
    ctx.lineTo(rearX + SHIP_SIZE * 0.02, -SHIP_SIZE * 0.02);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(noseX - SHIP_SIZE * 0.08, SHIP_SIZE * 0.02);
    ctx.lineTo(rearX + SHIP_SIZE * 0.02, SHIP_SIZE * 0.02);
    ...
    // Draw shield if active (unchanged)
